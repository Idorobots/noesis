...
var toggleMoveTrace = function( filter )
{
  if ( filter.moveCanvas.style.opacity =="0" )
  {
    ...
    $('moveTrace').update( 'Hide move trace' );
  }
  else
  {
    filter.moveCanvas.style.opacity = "0";
    $('moveTrace').update( 'Show move trace' );
  }
}
...
particleFilter.prototype = 
{
  ...
  draw: function( )
  {
   ...
   // this.particlesContext.clearRect( 0, 0 , this.width, this.height );
    ...
    for ( var i = 0, len = particles.length; i < len; ++i )
    {
      ...
      imageData.data[offset + 0] = 127;  // RED        
      imageData.data[offset + 1] = 127;  // GREEN  
      imageData.data[offset + 2] = 127;  // BLUE
  ...
  // clear space meaning all components more than 30, do not care about A
  isClearSpace: function( x, y )
  {
    if ( this.isOnCanvas( x, y ) )
    {
      var offset = ( y * this.imageData.width  + x ) * 4; // 4->RGBA
      return this.imageData.data[ offset ] > 30 && this.imageData.data[ offset + 2 ] > 30 
            && this.imageData.data[ offset + 4 ] > 30;
    }
    return false;
  },
...
particleFilter.robot.prototype = 
{
  ...
  draw: function( )
  {
    ...
    //this.drawCross( 2 );
    var ctx = this.filter.robotContext;
    //draw a circle
    ctx.beginPath();
    ctx.arc(this.xpx, this.ypx, 5, 0, Math.PI*2, true); 
    ctx.closePath();
